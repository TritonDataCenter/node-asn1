...
Reader.prototype.readByte = function(peek) {
  if (this._size - this._offset < 1) {
      throw new Error(`invalid byte from ${this._offset} with current size ${this._size}`);
      return null;
  }
...
Reader.prototype.readLength = function(offset) {
  ...
  if (offset >= this._size) {
      throw new Error(`offset ${offset} outside buffer size ${this._size}`);
      return null;
  }
...
Reader.prototype.readString = function(tag, retbuf) {
  ...
  var b = this.peek();
  if (b === null) {
      throw new Error(`Invalid tag - ${b} expected ${tag.toString(16)}`);
      return null;
  }
  if (b !== tag)
    throw newInvalidAsn1Error('Expected 0x' + tag.toString(16) +
  ...
  if (o === null) {
      throw new Error("readString - invalid length");
  }
    if (this.length === 0) {
        this._offset = o;
  ...
  if (this.length > this._size - o) {
      throw new Error (`invalid block length ${this.length} > ${this._size} - ${o}`);
  }
