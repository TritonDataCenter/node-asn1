...
class Writer {
    ...
    get buffer() {
        if (this._seq.length)
            throw new common_1.InvalidAsn1Error(this._seq.length + ' unended sequence(s)');
    ...
    writeLength(len) {
        ...
        if (len <= 0x7f) {
            ...
            throw new common_1.InvalidAsn1Error('Length too long (> 4 bytes)');
        }
    }
    ;
    writeByte(b) {
    ...
    writeInt(i, tag) {
        ...
        if (sz > 4)
            throw new common_1.InvalidAsn1Error('BER ints cannot be > 0xffffffff');
        this._ensure(2 + sz);
    ...
    ;
    writeNull() {
        this.writeByte(common_1.ASN1.Null);
        this.writeByte(0x00);
    }
    ;
    ...
    startSequence(tag) {
        if (typeof (tag) !== 'number')
            tag = common_1.ASN1.Sequence | common_1.ASN1.Constructor;
        this.writeByte(tag);
    ...
    endSequence() {
        ...
        if (len <= 0x7f) {
            ...
            throw new common_1.InvalidAsn1Error('Sequence too long');
        }
    }
    ;
